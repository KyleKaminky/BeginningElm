    ...
    describe "AND gate"
        [ test "output is 0 when both inputs are 0" <|
            \_ ->
                andGate 0 0
                    |> Expect.equal 0
        , test "output is 0 when first input is 0" <|
            \_ -> 
                andGate 0 1
                    |> Expect.equal 0
    ...
    describe "OR Gate"
        [ test "output is 0 when both inputs are 0" <|
            \_ ->
                orGate 0 0
                    |> Expect.equal 0
        , test "output is 1 when first inpuit is 1" <|
            \_ -> 
                orGate 1 0
                    |> Expect.equal 1
...
halfAdderTests = 
    describe "Half adder"
        [ describe "1st input is 0"
            [ test "and 2nd input is 0, the  sum and carry-out are 0" <|
                \_ ->
                    halfAdder 0 0
                        |> Expect.equal { carry = 0, sum = 0 }
            , test "and 2nd input is 1, sum is 1 and carry-out is 0" <|
                \_ ->
                    halfAdder 0 1
                        |> Expect.equal { carry = 0, sum = 1 }
            ]
        , describe "1st input is 1"       
            [ test "and 2nd input is 0, sum is 1 and carryout is 0"  <|
                \_ ->
                    halfAdder 1 0
                        |> Expect.equal { carry = 0, sum = 1 }
            , test "and 2nd input is 1, sum is 0 and carry-out is 1" <|
                \_ ->
                    halfAdder 1 1 
